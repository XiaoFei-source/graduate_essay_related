...
def c_line_read():
    T1 = open("/home/pi/Firstep/c_line.txt", "r")
    c_line = T1.readline()
    T1.close()
    c_line = float(c_line)
    return c_line
...
def result_t1_read():
    T1 = open("/home/pi/Firstep/test.txt", "r")
    ...
    try:
        while True:
            while ser.in_waiting:          # 若收到序列資料…
                ...
                global impedance_c
                impedance_c = data_raw.decode()   # 用預設的UTF-8解碼
                path = '/home/pi/Firstep/c_line.txt'
                f = open(path, 'w')
                # with open(path, 'w') as f:
                f.write(impedance_c+"歐姆".__str__())
                f.close()
                print('接收到的c線資料：', impedance_c)
                ##
    ...
    try:
        while True:
            while ser.in_waiting:          # 若收到序列資料…
                data_raw = ser.readline()  # 讀取一行
                global impedance_t1
                impedance_t1 = data_raw.decode()   # 用預設的UTF-8解碼
                path = '/home/pi/Firstep/test.txt'
                f = open(path, 'w')
                f.write(impedance_t1+"歐姆".__str__())
                f.close()
                print('接收到的t1線資料：', impedance_t1)
                raise KeyboardInterrupt
    except KeyboardInterrupt:
        ser.close()
       # return impedance_t1
    ...
    try:
        while True:
            while ser.in_waiting:          # 若收到序列資料…
                data_raw = ser.readline()  # 讀取一行
                global impedance_t2
                impedance_t2 = data_raw.decode()   # 用預設的UTF-8解碼
                path = '/home/pi/Firstep/test2.txt'
                f = open(path, 'w')
                # with open(path, 'w') as f:
                f.write(impedance_t2+"歐姆".__str__())
                f.close()
                print('接收到的t2線資料：', impedance_t2)
                ##
...
class StartPage(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        btn1 = tk.Button(self, text="test", compound=BOTTOM, font=28,
                         image=photo003, highlightthickness=0, bd=0, command=QRscanpage)
        btn2 = tk.Button(self, text="file", compound=BOTTOM, font=28,
                         image=photo002, highlightthickness=0, bd=0, command=filepage)
        btn3 = tk.Button(self, text="setting", compound=BOTTOM, font=28,
        ...
        self.label13 = tk.Label(self, text=3, font=1)
        self.label13.place(x=60, y=5)
        self.label14 = tk.Label(self, text=30, font=1)
    ...
    def update(self):
        ...
        self.label13.config(text='03' + '-')
        self.label14.config(text='30')
...
class QRscan(tk.Frame):
    def __init__(self, parent, controller):
        ...
        label1 = tk.Label(self, image=QRcode,
                          text="請掃描QRcode,以決定檢測品。", compound='top', font=('Helvetica', 24, "bold"))
        label1.place(x=8, y=120)
        entry1 = tk.Entry(self, font=('Helvetica', 60))
        entry1.place(x=50, y=420, width=350, height=120)
...
class Test(tk.Frame):
    def __init__(self, parent, controller):
        ...
        def run_t1():
            runtest = threading.Thread(name='Test_t1', target=testing_t1)
            runtest.start()
            print("電路板進行量測")
            time.sleep(30)
            # loading1()
        ...
        def run_t2():
            time.sleep(20)
            runtest = threading.Thread(name='Test_t2', target=testing_t2)
            runtest.start() 
            print("電路板進行量測")
            time.sleep(30)
            setup()
        ...
        label1 = tk.Label(self, text=pid, font="Courier 30")
        label1.place(x=270, y=190)  # 受測者id位置
...
class BluetoothExport(tk.Frame):
    def __init__(self, parent, controller):
        ...
        def loading():
            ...
            self.labelExport.place(x=2, y=85)
        ...
        btn1.place(x=30, y=60)  # export
        btn2.place(x=30, y=120)  # remove
        btn3.place(x=160, y=267)  # back
...
class File(tk.Frame):
    def __init__(self, parent, controller):
        ...
        def topLevel(seltext):  # Hi,I'm mini window XD.
            ...
            label2 = tk.Label(top, font="Courier 22")
            label2.place(x=10, y=30)
            label3 = tk.Label(top, font="Courier 22")
            label3.place(x=10, y=70)

            label5 = tk.Label(top, font="Courier 22", text="受測者編碼(ID):")
            label5.place(x=10, y=110)
            label6 = tk.Label(top, font="Courier 22")
            label6.place(x=250, y=110)
            label7 = tk.Label(top, font="Courier 22", text="品項:")
            label7.place(x=10, y=160)
            label8 = tk.Label(top, font="Courier 22")
            label8.place(x=10, y=210)
            label9 = tk.Label(top, font="Courier 22", text="C線是否有效:")
            label9.place(x=10, y=260)
            label10 = tk.Label(top, font="Courier 22")
            label10.place(x=10, y=310)
            label11 = tk.Label(top, font="Courier 28", text="T1_數值:")
            label11.place(x=10, y=360)
            label12 = tk.Label(top, font="Courier 28")
            label12.place(x=90, y=410)
            label13 = tk.Label(top, font="Courier 28", text="T2_數值:")
            label13.place(x=10, y=460)
            label14 = tk.Label(top, font="Courier 28")
            label14.place(x=90, y=510)
...
#####
            ...
            year1, month1, date1, hour1, minute1 = read_time()
            top.label12 = tk.Label(top, text=year1 + '-',  font=1)
            top.label13 = tk.Label(top, text=3 + '-',  font=1)
            top.label14 = tk.Label(top, text=30,  font=1)
            top.label15 = tk.Label(top, text=hour1 + ':',  font=1)
            top.label16 = tk.Label(top, text=minute1,  font=1)
...
class bluePrinter(tk.Frame):
    def __init__(self, parent, controller):
        ...
        btn1.place(x=110, y=420)  # ------尋找
        btn2.place(x=310, y=420)  # ------配對
        btn3.place(x=510, y=420)  # ------返回
...
class Settime(tk.Frame):
    ...
    def __init__(self, master, controller):
        ...
        self.label1.place(x=210, y=70)  # ----------------輸入年份
        self.label2.place(x=210, y=150)  # ----------------輸入月份
        self.label3.place(x=210, y=230)  # ----------------輸入日期
        self.label4.place(x=210, y=310)  # ----------------輸入小時
        self.label5.place(x=210, y=390)  # ----------------輸入分鐘
...
class keyboard(tk.Frame):
    def __init__(self, parent, controller):
        ...
        values.set('')
        entry = tk.Entry(self, textvariable=values,
                         width=17, font='Courier 19')
        entry.place(x=20, y=36)  # ------輸入框
